...
Dis_entry.grid(rowspan= 1 , columnspan = 1000 , ipadx = 999 , ipady = 20)
# end entry box
...
letters={'space':space,'a':A,'b':B,'c':C,"d":D,"e":E,"f":F,"g":G,"h":H,"i":I,"j":J,"k":K,"l":L,"m":M,"n":N,"o":O,"p":P,"q":q,"r":R,"s":S,"t":T,"u":U,"v":V,"w":w,"x":X,"y":Y,"z":Z}
def on_key_press(event):
    print("fuction called")
    print(event.keysym,letters[event.keysym],type(letters[event.keysym]))
   # letters[event.keysym].configure(bg='red')
    letters[event.keysym].invoke()
key.bind("<KeyPress>", on_key_press)



key.mainloop()  # using ending point

#  tell use how to diplay key press
# import tkinter as tk
#
# def on_key_press(event):
#     if event.keysym == "b":
